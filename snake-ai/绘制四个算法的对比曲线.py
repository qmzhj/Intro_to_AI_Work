...
Usage:
  python 绘制曲线.py  --compare experiments/dqn_20260516_101106 experiments/dueling_dqn_20260513_000142 experiments/double_dqn_20260513_112335 experiments/ppo_20260516_183650
"""
...
# 指定要比较的实验目录
SPECIFIED_EXPERIMENTS = [
    'experiments/dqn_20260516_101106',
    'experiments/dueling_dqn_20260513_000142', 
    'experiments/double_dqn_20260513_112335',
    'experiments/ppo_20260516_183650'
]
...
def main():
    ...
    # 如果用户没有指定实验，则使用预设的四个实验
    if not args.experiments and not args.compare:
        args.experiments = SPECIFIED_EXPERIMENTS
        args.compare = True
        args.output = 'algorithm_comparison.png'
        args.title = 'Comparison: DQN vs Dueling DQN vs Double DQN vs PPO'
    ...
    for exp_arg in args.experiments:
        ...
        if not os.path.exists(exp_path):
            print(f"Error: Experiment path does not exist: {exp_arg}")
            continue
        ...
        if result[0] is not None:  # Successfully loaded
            ...
            print(f"Error: Failed to load data from: {exp_arg}")
    ...
    if len(experiments_data) == 1 and not args.compare:
        ...
        if args.title == 'Training Curves Comparison':
            ...
            if len(exp_names) <= 3:
                ...
                title = f"Comparison of {len(exp_names)} Algorithms"
        else:
            title = args.title
        
        # 设置输出路径
        if args.output == 'comparison.png':
            output_path = 'algorithm_comparison.png'
        else:
            output_path = args.output
        
        plot_comparison(experiments_data, output_path, title)
